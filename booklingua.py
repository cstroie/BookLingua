...
class EPUBTranslator:
    ...
    def db_get_paragraphs(self, chapter_number: int, source_lang: str, target_lang: str) -> List[str]:
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute('''
                SELECT paragraph_number, source_text FROM translations 
                WHERE chapter_number = ? AND source_lang = ? AND target_lang = ? 
                ORDER BY paragraph_number ASC
            ''', (chapter_number, source_lang, target_lang))
            results = cursor.fetchall()
            # Return list of paragraph texts
            return [(result[0], result[1]) for result in results if result[1] is not None] if results else []
        except Exception as e:
            if self.verbose:
    ...
    def translate_epub(self, input_path: str, output_dir: str = "output", 
        ...
        if self.conn:
            try:
                ...
                # Clear the existing translated_chapters list
                translated_chapters = []
    ...
    def translate_chapter(self, chapter_number: int, source_lang: str, target_lang: str, total_chapters: int) -> epub.EpubHtml:
        ...
        if translated_count >= non_empty_paragraphs:
            ...
            for j, paragraph in enumerate(paragraphs):
                if self.verbose:
                    print(f"\nTranslating chapter {chapter_number}/{total_chapters}, paragraph {j+1}/{len(paragraphs)}")
